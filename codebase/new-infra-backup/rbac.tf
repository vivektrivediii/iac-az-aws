...
# }
##added by vivek
##admin access to tf or current user

resource "azurerm_role_assignment" "tf_runner_cluster_admin" {
  scope                = module.aks.cluster_id
  role_definition_name = "Azure Kubernetes Service Cluster Admin Role"
  principal_id         = data.azurerm_client_config.current.object_id
}

resource "azurerm_role_assignment" "tf_runner_keyvault_admin" {
  scope                = module.keyvault_infra.key_vault_id
  role_definition_name = "Key Vault Administrator"
  principal_id         = data.azurerm_client_config.current.object_id

  # Prevent Terraform from removing this before deleting the vault
  # lifecycle {
...
#   secret_permissions = ["Get", "List", "Delete", "Purge", "Set"]
#   key_permissions    = ["Get", "List", "Delete", "Purge"]

#   depends_on = [
#     azurerm_key_vault_secret.platform_secrets
#   ]
# }
